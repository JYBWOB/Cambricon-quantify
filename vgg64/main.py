...
is_quantifized = False
prototxt_path = './models/VGG7_64.prototxt'
data_path = './data/imgs/'
...
# input data
with open(data_path + 'image.lst') as f:
    imnames = f.readlines()
...
# labels = labels[:20]
 ...
 # import model 
if not is_quantifized:
    model = caffe.Net(model_path + '.prototxt', model_path + '.caffemodel', caffe.TEST)
    print('use unquantified model')
else:
    model = caffe.Net(model_path + '_qtz.prototxt', model_path + '.caffemodel', caffe.TEST)
    print('use quantified model')
